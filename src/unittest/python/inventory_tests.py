...
class TestBioGuard(unittest.TestCase):
    ...
    def test_range_temperature(self):
        # Nueva validación de límites térmicos (2 a 8 grados)
        self.assertTrue(check_temp(4))   # 4 está dentro del rango (debe ser True) 
        self.assertFalse(check_temp(0))  # 0 ahora está FUERA del rango (debe ser False) 
        self.assertFalse(check_temp(30)) # Caso por encima (debe ser False)